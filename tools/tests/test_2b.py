...
dki_content_1 = 'Пользователю нравится зелёный цвет'

# _extract_fragment should find the content in raw_text and return surrounding chars
fragment = compiler._extract_fragment(dki_content_1, raw_text_1)
print(f'  RAW: {raw_text_1}')
print(f'  Content: {dki_content_1}')
...
# The fragment should contain text from the RAW that relates to the DKI
assert any(word in raw_text_1.lower() for word in ['зелёный', 'нравится']), 'Fragment context check'
...
dki_content_2 = 'Пользователю нравится зелёный цвет'  # LLM paraphrase, not in RAW
...
assert fragment2 != raw_text_2[:150], 'Should NOT fabricate first 150 chars as fallback'
...
long_raw = 'В начале документа много нерелевантной информации о погоде и расписании. ' * 5 + \
           'В конце пользователь сказал что хочет тёмную тему.'
dki_content_3 = 'Пользователь предпочитает тёмную тему'
...
assert 'погода' not in (fragment3 or '').lower(), 'Should NOT contain unrelated beginning of document'
...
assert 'ask_ollama' in source_code or 'chat_router' in source_code, \
    '_call_llm should reuse existing ask_ollama interface from chat_router'
